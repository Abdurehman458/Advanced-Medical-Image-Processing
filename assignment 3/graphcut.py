...
def graph_cut(path = None):
    oimg = np.load(path+'/img.npy')
    oimg = oimg[1]
    oimg = convert(oimg, 0, 255, np.uint8)
    # oimg = cv2.imread('dog.jpg')
    ...
    N = h * w
    # x- column, y - row
    seg_label = np.load(path+'/seg.npy')
    fx, fy = gp.getpixels(img, 'fg',path, seg_label)
    bx, by = gp.getpixels(img, 'bg',path, seg_label)
    ...
    cv2.imwrite(path+'/graphcut.jpg', resgraph)
    cv2.imwrite(path+'/graphcut_svm.jpg', ressvm)
    # cv2.imwrite(str(lambdaa) + '_diff.jpg', diff)
