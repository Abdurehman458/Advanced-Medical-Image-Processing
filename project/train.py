...
class DS(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # seg_clip = np.expand_dims(seg_clip,0)
        # seg_clip = np.transpose(seg_clip,(1,2,0))
        # img = np.concatenate((img, np.expand_dims(seg_clip,0)),axis=0)
        img = img/img.max()
        img = np.transpose(img,(1,2,0)) 
        img = resize(img, (224,224,4))
        seg_clip = resize(seg_clip, (224,224))
        # img = Image.fromarray(img)
        # label = torch.from_numpy(label)
        img = self.transform(img)
        # seg_clip = self.transform(seg_clip) 

        return img,self.image_files_list[idx],seg_clip
...
class Trainer(object):
    def __init__(self, args):
        ...
        train_dataset = DS(base,datatype="train")
        val_dataset = DS(base,datatype="valid")
    ...
    def train(self):
        ...
        for epoch in range(self.args.start_epoch, self.args.epochs):
            ...
            for i, (images,_, targets) in enumerate(self.train_loader):
                cur_lr = self.lr_scheduler(cur_iters)
                for param_group in self.optimizer.param_groups:
                    param_group['lr'] = cur_lr

                images = images.to(self.args.device,dtype=torch.float32)
    ...
    def validation(self, epoch):
        ...
        self.metric.reset()
        self.model.eval()
        for i, (image,_, target) in enumerate(self.val_loader):
